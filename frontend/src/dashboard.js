...
async function updateUserNameAndImage(sessionUser) {
  const user = sessionUser || await apiGet('/user');
...
async function applyDashboardMapLocation(location, { forced = false } = {}) {
  ...
  if (!dashboardMiniMap) {
    ...
    dashboardMiniMap = new google.maps.Map(mapElement, {
      center,
      zoom: location.source === 'browser' || location.source === 'cached' ? 13 : 11,
    ...
    dashboardMiniMap.setZoom(location.source === 'browser' || location.source === 'cached' ? 13 : 11);
  }
  ...
  dashboardMiniMapMarker = createMapMarker({
    position: center,
  ...
  setText(
    ...
    forced
      ...
      : 'Default map preview. Use current location to centre this map on your position.'
  );
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const user = await requireAuthenticatedSession({ next: '/dashboard' });
  if (!user) {
    return;
  }

  await updateUserNameAndImage(user);
